"""
Video denoising inference script with selectable evaluator and multi-file support.
- All comments are in English.
- Set SELECT_EVAL to 1,2,3 or 4 to choose which evaluator runs.
- The list of input files is taken from unet3d.config.FILES and they are searched inside md.PATH_TO_INFER.
- Outputs are written into md.PATH_INFERED (created if not exists) with per-file suffixes.
"""
...
import unet3d.config as conf

# -----------------------------
# User hyperparameter: choose evaluator 1..4
# -----------------------------
SELECT_EVAL = 4  # set to 1, 2, 3 or 4
# -----------------------------
...
def read_all_frames(path):
    """Read all frames from an mp4 and return list of RGB uint8 numpy arrays and fps."""
...
def frames_to_batch_tensor(frames_rgb, device):
    """
    Convert a list of T frames (H,W,3) uint8 RGB to a torch tensor of shape (1,3,T,H,W),
    float32 normalized to [0,1], on the given device.
    """
...
def batch_tensor_to_rgb_uint8(batch_tensor):
    """
    Convert tensor (1,3,T,H,W) or (3,T,H,W) or (T,3,H,W) with values assumed in [0,1]
    to a list of RGB uint8 numpy arrays (length T).
    """
    ...
    if t.dim() == 4:
        ...
        # (T, H, W) single-channel? not expected
...
def infer_window(model, window_frames_rgb, device):
    ...
    Returns a list of 4 RGB uint8 numpy arrays (model output), values assumed in [0,1] before conversion.
    """
    ...
    out_frames = batch_tensor_to_rgb_uint8(out)  # list of 4 RGB uint8 np arrays
    return out_frames
...
def block_evaluator(model, frames, device):
    ...
    Returns a list of RGB uint8 frames of length equal to original number of frames.
    """
    N = len(frames)
    if N == 0:
        return []
    ...
    while i + 4 <= N:
        window = frames[i:i+4]  # exactly 4
        out_frames = infer_window(model, window, device)  # list of 4 rgb arrays
        for t in range(4):
            out_seq[i + t] = out_frames[t]
        i += 4

    # if there are leftover frames at the end (i < N), evaluate last 4 frames and append only missing ones
    if i < N:
        start = max(0, N - 4)
        last_window = frames[start:start+4]
        out_last = infer_window(model, last_window, device)
        for idx in range(i, N):
            rel = idx - start  # position in last_window (0..3)
...
def sliding_center_evaluator(model, frames, device):
    """
    Evaluator 2: sliding windows with stride=2. For each window we keep ONLY the two central output frames.
    Mapping rule: for a window starting at s, the model outputs frames out[0..3]; we map:
        out[1] -> index s
        out[2] -> index s + 1
    The first frame of the video is guaranteed to be produced by the very first inference:
      - if a valid window starting at 0 exists, it is used;
      - otherwise a padded window starting at 0 is run first (so out[1] supplies frame 0).
    Tail frames are fixed by running an inference on the last 4 frames (if any positions remain unset).
    """
    N = len(frames)
    out_seq = [None] * N
    ...
    # Ensure we always process a window starting at 0 first (padded if needed) so frame 0 comes from first inference
    if 0 not in valid_starts:
        # run a padded window at 0
        win0 = frames[0:4]
        ...
        # we'll process start 0 in the normal loop
        pass
    ...
    # Fix tail: if some frames at the end are still None, evaluate last 4 frames and fill missing tail frames
    first_missing = None
    for idx in range(N):
...
def sliding_mean_stride2_evaluator(model, frames, device):
    """
    Evaluator 3: sliding windows with stride=2. For each window starting at s, map output frame t to original index s+t.
    Accumulate sums and counts for each original frame index and average at the end.
    If there are indexes never predicted (due to edge rounding), evaluate last 4 frames and include them.
    """
    N = len(frames)
    if N == 0:
        return []
    ...
    for s in tqdm(valid_starts, desc="sliding_mean_stride2 eval", leave=False):
        out_frames = infer_window(model, frames[s:s+4], device)  # list of 4 rgb arrays
        for t in range(4):
            idx = s + t
            if idx >= N:
                continue
    ...
    # Tail handling: if some indices have zero count, run last-4 inference and add them
    start = max(0, N - 4)
    out_last = infer_window(model, frames[start:start+4], device)
    for t in range(4):
        idx = start + t
    ...
    # Build averaged frames. If for some reason count==0, fallback to original frame.
    out_seq = []
    for i in range(N):
...
def sliding_mean_stride1_evaluator(model, frames, device):
    """
    Evaluator 4: sliding windows with stride=1. For every window starting at s (0..N-4) evaluate and map output t->s+t,
    accumulate and average all available predictions per original frame.
    """
    N = len(frames)
    if N == 0:
        return []
    ...
    starts = list(range(0, max(1, N)))  # will iterate 0..N-1 but we only use windows where s+4<=N
    valid_starts = [s for s in starts if s + 4 <= N]

    for s in tqdm(valid_starts, desc="sliding_mean_stride1 eval", leave=False):
...
if __name__ == "__main__":
    ...
    ckpt = torch.load(os.path.join(md.PATH_MODELS, "unetres16.pth"), map_location=device)
    model.load_state_dict(ckpt["model_state"])
    model.eval()

    evaluator_fn, suffix = EVALUATOR_MAP[SELECT_EVAL]
